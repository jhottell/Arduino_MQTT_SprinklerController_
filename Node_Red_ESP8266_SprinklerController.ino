...
const char* password = "SecureP@ssword";
const char* mqtt_server = "10.100.100.180";
const char* mqtt_topic = "Sprinkler/";
const char* mqtt_user = "user";
const char* mqtt_password = "P@ssword";

char vInp13 = 0;
String rx;
int  rxLength = 0;
int timer1Setting = 500; //600
int timer2Setting = 500; //600
int timer3Setting = 240; //300
int timer4Setting = 240; //300
int timer1 = 0;
int timer2 = 0;
int timer3 = 0;
int timer4 = 0;
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    //if (client.connect(mqtt_topic)) {
      ...
      client.publish(mqtt_topic, "Sprinkler Controller Online");
      // ... and resubscribe
...
void outputs()
{
  ...
  if (run1 == 1 and firstOut == 0)
  {
    ...
    client.publish("Sprinkler/Run1", "1");
    run1Stat = 1;
    timer1 = 1;
    firstOutSub();
  }
  else if (run1 == 0 and run1Stat == 1)
  {
    ...
    client.publish("Sprinkler/Run1", "0");
    run1Stat = 0;
    timer1 = 0;
    firstOutSub();
  }
  ...
  if (run2 == 1 and firstOut == 0)
  {
    ...
    client.publish("Sprinkler/Run2", "1");
    run2Stat = 1;
    timer2 = 1;
    firstOutSub();
  }
  else if (run2 == 0 and run2Stat == 1)
  {
    ...
    client.publish("Sprinkler/Run2", "0");
    run2Stat = 0;
    timer2 = 0;
    firstOutSub();
  }
  ...
  if (run3 == 1 and firstOut == 0)
  {
    ...
    client.publish("Sprinkler/Run3", "1");
    run3Stat = 1;
    timer3 = 1;
    firstOutSub();
  }
  else if (run3 == 0 and run3Stat == 1)
  {
    ...
    client.publish("Sprinkler/Run3", "0");
    run3Stat = 0;
    timer3 = 0;
    firstOutSub();
  }
  ...
  if (run4 == 1 and firstOut == 0)
  {
    ...
    client.publish("Sprinkler/Run4", "1");
    run4Stat = 1;
    timer4 = 1;
    firstOutSub();    
  }
  else if (run4 == 0 and run4Stat == 1)
  {
    ...
    client.publish("Sprinkler/Run4", "0");  
    run4Stat = 0;
    timer4 = 0;
    firstOutSub();    
  }
